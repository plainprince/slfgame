...
io.on('connection', (socket) => {
    ...
    socket.on('startGameRound', (data) => {
        ...
        // Clear any existing timer (removed automatic time limit)
        if (game.roundTimer) {
    ...
    socket.on('submitAnswers', (data) => {
        ...
        // If all players submitted, end the round
        if (submittedCount >= totalPlayers) {
            endRound(gameId);
        }
    });

    // stop round (only game creator can stop)
    socket.on('stopRound', (data) => {
        ...
        // This gives them a chance to auto-submit their current answers
        io.to(gameId).emit('roundStopping');

        // Wait a short moment for auto-submissions, then end the round
        setTimeout(() => {
            endRound(gameId);
        }, 500); // 500ms delay to allow auto-submissions
    });
